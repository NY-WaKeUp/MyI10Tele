...
from core.my_policy import MyPolicy, resolve_pi0_pretrained_path
...
_CKPT_SUBDIR = f"{_pi_pretrained_id.split('/')[-1]}/{DATASET_NAME}"

POLICY_PATH: str | Path | None = (
    None  # None = auto-find .ckpt/*/DATASET_NAME with type pi0
)
...
N_EPISODES = 50
BATCH_SIZE = 10  # parallel sim envs (CPU-bound); not training micro-batch
SEED = 1000
USE_AMP = True
...
def _task_prompt_from_dataset() -> str:
    """Same language string as LeRobotDataset batches (PI0NewLineProcessor adds '\\n')."""
...
def _make_eval_processors(
    ...
    print(
        "No policy_preprocessor.json in checkpoint; building PI0 processors from dataset stats "
        f"({DATASET_ROOT})."
    )
...
def _wrap_preprocessor_with_task(
    ...
    """gym_aloha has no task_description; PI0 must see the same prompt as training."""

    def _run(obs: dict[str, Any]) -> dict[str, Any]:
        n = obs[next(iter(obs))].shape[0]
        obs = dict(obs)
