...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Passive MuJoCo viewer + optional onboard camera preview.")
    p.add_argument("--no-camera-windows", action="store_true", help="Disable OpenCV camera preview window.")
    p.add_argument("--cam-width", type=int, default=320, help="Width per camera panel (pixels).")
    p.add_argument("--cam-height", type=int, default=240, help="Height per camera panel (pixels).")
    p.add_argument(
        "--camera-display",
...
def main() -> None:
    ...
    i10_inspire_flange_link_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "i10_inspire_flange_link")
    wrist3_joint_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, "wrist3_joint")
    ...
    if not args.no_camera_windows:
        if agentview_id < 0:
            print("warn: camera 'agentview' not found; disable preview or add table.xml camera.")
        if wrist_cam_id < 0:
            print("warn: camera 'wrist_cam' not found; disable preview or add wrist_cam in aubo_i10_inspire.xml.")
    ...
    if show_cams and display_mode == "process":
        ...
        preview_proc = mp.Process(target=opencv_preview_worker.run, args=(frame_queue,), daemon=False)
        preview_proc.start()
    ...
    if show_cams:
        ...
        print(f"camera preview ({how}): agentview | wrist_cam — close MuJoCo viewer to quit")
        ...
        with viewer.launch_passive(model, data) as v:
            ...
            while v.is_running():
                ...
                if step_count % 60 == 0:
                    ...
                    print(f"ee_site xyz (m): [{ee_pos[0]: .4f}, {ee_pos[1]: .4f}, {ee_pos[2]: .4f}]")
                    ...
                    print(f"wrist3_anchor xyz (m): [{wrist3_anchor[0]: .4f}, {wrist3_anchor[1]: .4f}, {wrist3_anchor[2]: .4f}]")
                step_count += 1

                if show_cams and renderer is not None and cam_agent is not None and cam_wrist is not None:
                    ...
                    cv2.putText(panel_a, "agentview", (8, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
                    cv2.putText(panel_w, "wrist_cam", (8, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
